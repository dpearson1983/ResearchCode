...
__constant__ float4 d_Pkbao[128]; //  16384 bytes
...
//           total constant memory:   35584 bytes out of 65536 bytes (only 49152 bytes?)
...
__global__ void bispec_gauss_32_nw(float3 *ks, double *Bk);
...
// Enables the above bispec_model to be executed on many CUDA cores to speed up the integral calculation
__global__ void bispec_gauss_32_bao(float3 *ks, double *Bk);
...
__global__ void bispec_gauss_32_bao(float3 *ks, double *Bk) {
    ...
    // Calculate the value for this thread
    float phi = PI*d_xi[threadIdx.y] + PI;
    int_grid[tid] = d_wi[threadIdx.x]*d_wi[threadIdx.y]*bispec_model_bao(threadIdx.x, phi, 
                                                                         ks[blockIdx.x]);
    __syncthreads();
...
void bkmcmc::bk_norm() {
    for (int i = 0; i < bkmcmc::num_data; ++i) {
        ...
        bkmcmc::Bk_bao[i] /= bkmcmc::Bk_nw[i];
    }
}
...
void bkmcmc::model_calc(std::vector<double> &pars, float3 *ks, double *Bk) {
    ...
    bispec_gauss_32_bao<<<bkmcmc::num_data, num_threads>>>(ks, Bk);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMemcpy(bkmcmc::Bk_bao.data(), Bk, bkmcmc::num_data*sizeof(double), 
                         cudaMemcpyDeviceToHost));
    
    bispec_gauss_32_nw<<<bkmcmc::num_data, num_threads>>>(ks, Bk);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
