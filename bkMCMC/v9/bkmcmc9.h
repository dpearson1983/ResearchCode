...
// Enables the above bispec_model to be executed on many CUDA cores to speed up the integral calculation
__global__ void bispec_gauss_32_data(float3 *ks, double *Bk);

class bkmcmc{
    int num_data, num_pars;
    std::vector<double> data, data_norm, Bk_bao, Bk_nw, Psi, Psi_norm; // These should have size of num_data
    ...
    // Divides the data and the bao bispectra by the nw bispectrum;
    void bk_norm(); // done
...
__device__ double bispec_model(int x, float &phi, float3 k) {
    ...
    float P1bao = spline_eval_bao(k1)/(d_p[4]*d_p[4]*d_p[3]);
    float P2bao = spline_eval_bao(k2)/(d_p[4]*d_p[4]*d_p[3]);
    float P3bao = spline_eval_bao(k3)/(d_p[4]*d_p[4]*d_p[3]);
    
    float P1nw = spline_eval_nw(k1)/(d_p[4]*d_p[4]*d_p[3]);
    float P2nw = spline_eval_nw(k2)/(d_p[4]*d_p[4]*d_p[3]);
    float P3nw = spline_eval_nw(k3)/(d_p[4]*d_p[4]*d_p[3]);
...
__global__ void bispec_gauss_32_data(float3 *ks, double *Bk) {
    ...
    // Calculate the value for this thread
    float phi = PI*d_xi[threadIdx.y] + PI;
    int_grid[tid] = d_wi[threadIdx.x]*d_wi[threadIdx.y]*bispec_model_nw(threadIdx.x, phi, 
                                                                         ks[blockIdx.x]);
    __syncthreads();
...
void bkmcmc::bk_norm() {
    for (int i = 0; i < bkmcmc::num_data; ++i) {
        bkmcmc::data_norm[i] = bkmcmc::data[i]/bkmcmc::Bk_nw[i];
        double sigma = sqrt(bkmcmc::Psi[i])/bkmcmc::Bk_nw[i];
        bkmcmc::Psi_norm[i] = 1.0/(sigma*sigma);
    }
}

void bkmcmc::model_calc(std::vector<double> &pars, float3 *ks, double *Bk) {
    ...
    bispec_gauss_32<<<bkmcmc::num_data, num_threads>>>(ks, Bk);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMemcpy(bkmcmc::Bk_bao.data(), Bk, bkmcmc::num_data*sizeof(double), 
                         cudaMemcpyDeviceToHost));
    
    bispec_gauss_32_data<<<bkmcmc::num_data, num_threads>>>(ks, Bk);
    gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMemcpy(bkmcmc::Bk_nw.data(), Bk, bkmcmc::num_data*sizeof(double), 
                         cudaMemcpyDeviceToHost));
...
double bkmcmc::calc_chi_squared() {
    double chisq = 0.0;
    for (int i = 0; i < bkmcmc::num_data; ++i) {
        chisq += (bkmcmc::data_norm[i] - bkmcmc::Bk_bao[i])*Psi_norm[i]*(bkmcmc::data_norm[i] - bkmcmc::Bk_bao[i]);
    }
    return chisq;
}

bool bkmcmc::trial(float3 *ks, double *Bk, double &L, double &R) {
    bkmcmc::get_param_real();
    bkmcmc::model_calc(bkmcmc::theta_i, ks, Bk);
...
void bkmcmc::burn_in(int num_burn, float3 *ks, double *Bk) {
    std::cout << "Buring the first " << num_burn << " trials to move to higher likelihood..." << std::endl;
    double L, R;
    for (int i = 0; i < num_burn; ++i) {
        bool move = bkmcmc::trial(ks, Bk, L, R);
        if (move) {
            std::cout << "\r";
            std::cout.width(5);
            std::cout << i;
...
void bkmcmc::tune_vars(float3 *ks, double *Bk) {
...
bkmcmc::bkmcmc(std::string data_file, std::vector<double> &pars, std::vector<double> &vars, float3 *ks, 
    ...
    while (!fin.eof()) {
        ...
        if (!fin.eof()) {
            ...
            bkmcmc::Psi.push_back(var);
            bkmcmc::Psi_norm.push_back(0.0);
            bkmcmc::Bk_bao.push_back(0.0);
            bkmcmc::Bk_nw.push_back(0.0);
            bkmcmc::data_norm.push_back(0.0);
        }
    }
    fin.close();
    ...
    gpuErrchk(cudaMemcpy(Bk, bkmcmc::Bk_bao.data(), bkmcmc::num_data*sizeof(double), 
    ...
    bkmcmc::model_calc(bkmcmc::theta_0, ks, Bk);
    ...
    for (int i =0; i < bkmcmc::num_data; ++i) {
        ...
        fout.precision(15);
        fout << bkmcmc::data_norm[i] << " " << bkmcmc::Bk_bao[i] << "\n";
    }
    fout.close();
}
...
void bkmcmc::check_init() {
    ...
    std::cout << "    data_norm.size() = " << bkmcmc::data_norm.size() << std::endl;
    std::cout << "    Bk_bao.size()    = " << bkmcmc::Bk_bao.size() << std::endl;
    std::cout << "    Bk_nw.size()     = " << bkmcmc::Bk_nw.size() << std::endl;
...
void bkmcmc::run_chain(int num_draws, std::string reals_file, float3 *ks, double *Bk, bool new_chain) {
    int num_old_rels = 0;
    if (new_chain) {
        std::cout << "Starting new chain..." << std::endl;
        bkmcmc::burn_in(10000, ks, Bk);
        bkmcmc::tune_vars(ks, Bk);
    } else {
