...
double bkmcmc::get_model_power(std::vector<double> &pars, double k_i) {
    double P_nw = gsl_spline_eval(bkmcmc::Pk_nw, k_i/pars[3], bkmcmc::acc_nw);
    double P_bao = gsl_spline_eval(bkmcmc::Pk_bao, k_i/pars[3], bkmcmc::acc_bao);
...
// #pragma omp parallel for private(bkmcmc::Pk_bao, bkmcmc::acc_bao, bkmcmc::Pk_nw, bkmcmc::acc_nw)
    for (int i = 0; i < bkmcmc::num_data; ++i) {
        double k1 = bkmcmc::k[i].x;
        double k2 = bkmcmc::k[i].y;
        double k3 = bkmcmc::k[i].z;
        double P_1 = get_model_power(pars, k1);
        double P_2 = get_model_power(pars, k2);
        double P_3 = get_model_power(pars, k3);
        double mu12 = (k1*k1 + k2*k2 - k3*k3)/(2.0*k1*k2);
        double mu23 = (k2*k2 + k3*k3 - k1*k1)/(2.0*k2*k3);
        double mu31 = (k3*k3 + k1*k1 - k2*k2)/(2.0*k3*k1);
        double F12 = (5.0/7.0 - mu12*(k1/k2 + k2/k1) + (2.0/7.0)*mu12*mu12);
        double F23 = (5.0/7.0 - mu23*(k2/k3 + k3/k2) + (2.0/7.0)*mu23*mu23);
        double F31 = (5.0/7.0 - mu31*(k3/k1 + k1/k3) + (2.0/7.0)*mu31*mu31);
        double G12 = (3.0/7.0 - mu12*(k1/k2 + k2/k1) + (4.0/7.0)*mu12*mu12);
        double G23 = (3.0/7.0 - mu23*(k2/k3 + k3/k2) + (4.0/7.0)*mu23*mu23);
        double G31 = (3.0/7.0 - mu31*(k3/k1 + k1/k3) + (4.0/7.0)*mu31*mu31);
        double Z1_12 = pars[0]*pars[0] + pars[0]*pars[2] + (pars[2]*pars[2])/15.0 + 
                        (2.0*pars[2]*pars[2]*mu12*mu12)/15.0;
        double Z1_23 = pars[0]*pars[0] + pars[0]*pars[2] + (pars[2]*pars[2])/15.0 + 
