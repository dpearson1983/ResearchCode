...
struct mcmc_parameters{
    int num_params, N, num_threads, num_chains, num_burn, num_draws;
    double nbar, mcmc_tolerance;
    bool full_covar, new_chain;
    std::vector<double> start_params;
    std::string input_bao_power, input_nw_power, data_file, reals_file, param_covar_file, summary_file, cov_file;
