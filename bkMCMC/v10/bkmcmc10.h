...
class bkmcmc{
    ...
    std::vector<double> data, Bk_bao; // These should have size of num_data
...
double bkmcmc::calc_chi_squared() {
    ...
    for (int i = 0; i < bkmcmc::num_data; ++i) {
        for (int j = i; j < bkmcmc::num_data; ++j) {
            chisq += (bkmcmc::data[i] - bkmcmc::Bk_bao[i])*Psi[i][j]*(bkmcmc::data[j] - bkmcmc::Bk_bao[j]);
        }
    }
    return chisq;
}
...
bkmcmc::bkmcmc(std::string data_file, std::string cov_file, std::vector<double> &pars, std::vector<double> &vars, float3 *ks, 
               double *Bk) {
    std::ifstream fin;
    std::ofstream fout;
    ...
    for (int i =0; i < bkmcmc::num_data; ++i) {
        ...
        fout.precision(15);
        fout << bkmcmc::data[i] << " " << bkmcmc::Bk_bao[i] << "\n";
    }
    fout.close();
}
